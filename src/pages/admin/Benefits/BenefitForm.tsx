...
import { getBenefit, createBenefit, updateBenefit } from '../../../lib/admin/benefits';
...
export function BenefitForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        notify({ type: 'success', message: 'Beneficio actualizado correctamente' });
      } else {
        await createBenefit(payload);
        notify({ type: 'success', message: 'Beneficio creado correctamente' });
      }
      navigate('/admin/benefits');
    } catch (e) {
      notify({ type: 'error', message: getErrorMessage(e) });
  ...
  return (
    <div>
      <PageHeader
        title={isEdit ? 'Editar Beneficio' : 'Nuevo Beneficio'}
      ...
      <form onSubmit={handleSubmit} className="max-w-2xl space-y-6">
        ...
        <div className="flex items-center gap-3 pt-4 border-t border-gray-200">
          ...
          >
            {saving ? 'Guardando...' : (isEdit ? 'Actualizar Beneficio' : 'Crear Beneficio')}
          </button>
          <button
            type="button"
