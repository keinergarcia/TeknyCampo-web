...
import { listHeroStats, deleteHeroStat, toggleHeroStat } from '../../lib/admin/hero-stats';
import { getErrorMessage } from '../../lib/errors';
import type { HeroStat } from '../../types/admin';

interface UseHeroStatsReturn {
  data: HeroStat[];
  total: number;
  loading: boolean;
  error: string | null;
...
export function useHeroStats(): UseHeroStatsReturn {
  const [data, setData] = useState<HeroStat[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await listHeroStats({ page, pageSize, search, sortColumn, sortDirection });
      setData(result.data);
  ...
  const handleToggle = useCallback(async (id: string, active: boolean) => {
    await toggleHeroStat(id, active);
