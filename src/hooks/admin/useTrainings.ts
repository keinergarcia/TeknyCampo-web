...
import { listTrainings, deleteTraining, toggleTraining } from '../../lib/admin/trainings';
...
export function useTrainings(): UseTrainingsReturn {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await listTrainings({ page, pageSize, search, sortColumn, sortDirection });
      setData(result.data);
  ...
  const handleToggle = useCallback(async (id: string, active: boolean) => {
    await toggleTraining(id, active);
